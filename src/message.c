...
#define COMMAND_CONFIG_BAR_SPACE_ICON_COLOR_SECONDARY                          \
  "space_icon_color_secondary"
...
#define COMMAND_CONFIG_BAR_CENTER_SHELL_ICON "center_shell_icon"
#define COMMAND_CONFIG_BAR_CENTER_SHELL_ICON_COLOR "center_shell_icon_color"
...
#define COMMAND_CONFIG_BAR_DISPLAY_SEPARATOR_ICON_COLOR                        \
  "display_separator_icon_color"
...
static void handle_domain_config(FILE *rsp, struct token domain,
                                 char *message) {
  ...
  if (token_equals(command, COMMAND_CONFIG_DEBUG_OUTPUT)) {
    ...
    if (!token_is_valid(value)) {
      ...
      daemon_fail(
          rsp,
          "unknown value '%.*s' given to command '%.*s' for domain '%.*s'\n",
          value.length, value.text, command.length, command.text, domain.length,
          domain.text);
    }
    ...
    if (!token_is_valid(value)) {
      ...
      if (color) {
        ...
        daemon_fail(
            rsp,
            "unknown value '%.*s' given to command '%.*s' for domain '%.*s'\n",
            value.length, value.text, command.length, command.text,
    ...
    if (buf_len(g_bar_manager._power_icon_strip) != 2) {
      daemon_fail(rsp,
                  "value for '%.*s' must contain exactly two symbols separated "
                  "by whitespace.\n",
      ...
      if (color) {
        ...
        daemon_fail(
            ...
            domain.length, domain.text);
                          ...
                          COMMAND_CONFIG_BAR_SPACE_ICON_COLOR_SECONDARY)) {
    struct token value = get_token(&message);
    if (!token_is_valid(value)) {
      ...
      if (color) {
        ...
        daemon_fail(
            ...
            domain.length, domain.text);
                          ...
                          COMMAND_CONFIG_BAR_SPACE_ICON_COLOR_TERTIARY)) {
    struct token value = get_token(&message);
    if (!token_is_valid(value)) {
    ...
    if (!token_is_valid(token)) {
      ...
      bar_manager_set_padding_left(&g_bar_manager,
                                   atoi(token_to_string(token)));
    }
    ...
    if (!token_is_valid(token)) {
      ...
      bar_manager_set_padding_right(&g_bar_manager,
                                    atoi(token_to_string(token)));
    }
    ...
    if (!token_is_valid(token)) {
      ...
      bar_manager_set_spacing_left(&g_bar_manager,
                                   atoi(token_to_string(token)));
    }
    ...
    if (!token_is_valid(token)) {
      ...
      bar_manager_set_spacing_right(&g_bar_manager,
                                    atoi(token_to_string(token)));
    }
    ...
    if (!token_is_valid(value)) {
      ...
      daemon_fail(
          ...
          domain.text);
                          ...
                          COMMAND_CONFIG_BAR_SPACES_FOR_ALL_DISPLAYS)) {
    struct token value = get_token(&message);
    if (!token_is_valid(value)) {
    ...
    if (length <= 0) {
      ...
      bar_manager_set_center_shell_command(&g_bar_manager,
                                           string_copy(message));
    }
  ...
  } else if (token_equals(command, COMMAND_CONFIG_BAR_CENTER_SHELL_ICON_COLOR)) {
    struct token value = get_token(&message);
    if (!token_is_valid(value)) {
      fprintf(rsp, "0x%x\n", g_bar_manager.center_shell_icon_color.p);
    } else {
      uint32_t color = token_to_uint32t(value);
      if (color) {
        bar_manager_set_center_shell_icon_color(&g_bar_manager, color);
      } else {
        daemon_fail(
            rsp,
            "unknown value '%.*s' given to command '%.*s' for domain '%.*s'\n",
  ...
  } else if (token_equals(command, COMMAND_CONFIG_BAR_CENTER_SHELL_ICON)) {
    struct token token = get_token(&message);
    if (!token_is_valid(token)) {
      fprintf(rsp, "%s\n",
              g_bar_manager._center_shell_icon ? g_bar_manager._center_shell_icon
                                              : "");
    } else {
      bar_manager_set_center_shell_icon(&g_bar_manager, token_to_string(token));
    }
    ...
    if (!token_is_valid(value)) {
      ...
      daemon_fail(
          ...
          domain.text);
                          ...
                          COMMAND_CONFIG_BAR_DISPLAY_SEPARATOR_ICON_COLOR)) {
    struct token value = get_token(&message);
    if (!token_is_valid(value)) {
    ...
    if (!token_is_valid(token)) {
      fprintf(rsp, "%s\n",
              g_bar_manager._display_separator_icon
                  ? g_bar_manager._display_separator_icon
                  : "");
    } else {
      bar_manager_set_display_separator_icon(&g_bar_manager,
...
static SOCKET_DAEMON_HANDLER(message_handler) {
  struct event *event =
      event_create_p1(&g_event_loop, DAEMON_MESSAGE, message, sockfd);
